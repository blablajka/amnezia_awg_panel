...
class Bridge(Base):
    """
    Мост/туннель между двумя серверами.

    Используется для каскадной схемы:
    Клиент → Русский VPS (server_from) → Зарубежный VPS (server_to)

    Поддерживаемые протоколы туннеля: awg, hysteria2, gost
    """
    ...
    protocol: Mapped[str] = mapped_column(
        String(50), default="gost", nullable=False,
    )  # awg, hysteria2, gost
    config_data: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # Конфигурация туннеля
    is_active: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
        server_default=func.now(),
    ...
    def __repr__(self) -> str:
        return f"<Bridge id={self.id} {self.server_from_id}->{self.server_to_id} [{self.protocol}]>"
